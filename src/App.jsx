...
let required = isWE? [{...weekendShift}] : [...weekdayShifts];
      ...
      if(isWE){
        if(date.getDay()===6){ // sábado
          ...
          generateSchedule._carry={ sunday: toDateValue(addDays(date,1)), personId: weekendFixedId }} else { // domingo
      ...
      for(let s=0; s<required.length; s++){
        ...
        if(!chosen && isWE && s===0 && weekendFixedId && pool.some(p=>p.id===weekendFixedId)) {
          chosen=weekendFixedId} else if(!chosen && isWE && s===0 && !weekendFixedId){
          const prefer=pool.find(p=>p.id===nextOff);
          chosen=prefer?.id || pickBestCandidate(pool,{isWeekend:isWE,weekdaysLoad,weekendLoad,priorityMap})} else if(!chosen){
        ...
        if(chosen){
          ...
          dayAssignments.push({shift, personId:chosen, conflict:false})} else {
...
async function cloudSave() { setUI(prev=>({...prev, sync:"loading"}));
    try{
      ...
      setUI(prev=>({...prev, sync:"ok"})); showToast("Guardado en nube")}catch(e){ setUI(prev=>({...prev, sync:"error"})); showToast((String(e.message||"")).startsWith("403")?"403: ReadToken inválido o sin permisos":"Error al cargar: "+e.message)}
  }
  ...
  useEffect(()=>{
    ...
    const s = parseDateValue(state.startDate);
    const idx = Math.max(0, Math.min(state.weeks-1, Math.floor((t - s)/(7*24*3600*1000))));
    if (Number.isFinite(idx) && idx !== weekIndex) setWeekIndex(idx)}, [state.startDate, state.weeks]);
                    ...
                    return (
                      <div key={i} className={`rounded-xl px-2 py-1.5 border text-sm flex items-center justify-between ${c.conflict? 'border-red-300 bg-red-50':'border-slate-200'}`} title={`${lbl} · ${span} (${dur}h)`}>
                        ...
                        <div className="flex items-center gap-1">
                          {p ? (
                            <span className="chip inline-flex items-center gap-1 px-1.5 py-0.5 rounded-lg" style={{background:`${p.color}20`, border:`1px solid ${p.color}55`}}>
...
function AdminUsersAndPerms({ auth }) {
  ...
  async function loadUsers() {
    try {
      ...
      setUsers(data.users || [])} catch (e) {
      alert(e.message)} finally {
      setLoading(false)}
  }
  ...
  async function resetPassword(email) {
    ...
    try {
      ...
      alert('Contraseña actualizada')} catch (e) {
      alert(e.message)} finally {
      setLoading(false)}
  }
  ...
  async function transferOwnership() {
    ...
    try {
      ...
      alert('Propiedad transferida')} catch (e) {
      alert(e.message)} finally {
      setLoading(false)}
  }
  ...
  async function updateTokens() {
    ...
    try {
      ...
      alert('Tokens actualizados')} catch (e) {
      alert(e.message)} finally {
      setLoading(false)}
  }
...
function snapToAllowedMonth(dateStr, vp){
  ...
  for (let delta=1; delta<=12; delta++){
    ...
    if (allow){
      ...
      if (set.has(down)) return cand(down)} else {
      if (!set.has(up))   return cand(up);
