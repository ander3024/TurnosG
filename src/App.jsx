...
function ReglasPanel({ state, up, isAdmin }){ if (!isAdmin) return null;
  return (
    <Card title="Reglas duras del convenio">
      <div className="grid grid-cols-12 gap-2">
        ...
        <div className="col-span-6">
          <label className="text-xs">Máx días trabajados/semana</label>
          <input type="number" min={1} max={7}
                 value={state.rules.maxDaysPerWeek ?? 5}
                 onChange={(e)=>up(['rules','maxDaysPerWeek'], Number(e.target.value))}
                 className="w-full px-2 py-1 rounded border" />
        </div>
        <div className="col-span-6">
          <label className="text-xs flex items-center gap-2">
            <input type="checkbox"
                   checked={(state.rules.maxConsecutiveWeekends||0) > 0}
                   onChange={(e)=>up(['rules','maxConsecutiveWeekends'], e.target.checked ? 1 : 0)} />
            Evitar fines de semana consecutivos
          </label>
          <div className="text-[11px] text-slate-500 mt-1">
            Si hay alternativas, no asigna el finde a quien lo trabajó la semana anterior.
          </div>
        </div>
